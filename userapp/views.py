...
items = [
    {
        'id': '1',
        'name': 'Аномальный перенос вещества в двухзонной фрактальной среде',
        'file': 'pdf_files/Аномальный перенос вещества в двухзонной фрактальной среде.pdf',
        'desc': 'Численное решение с дробными производными по времени (α) и пространству (β) в двухзонной пористой среде.',
        'color': 'blue',
    },
    {
        'id': '2',
        'name': 'Аномальный перенос с учетом адсорбционных эффектов и разложения вещества',
        'file': 'pdf_files/Аномальный перенос с учетом адсорбционных эффектов и разложения вещества.pdf',
        'desc': 'Модель учитывает адсорбцию и разложение вещества в mobile/immobile зонах пористой среды.',
        'color': 'purple',
    },
    {
        'id': '3',
        'name': 'Аномальный перенос с много – членными дробными производными',
        'file': 'pdf_files/Аномальный перенос с много – членными дробными производными.pdf',
        'desc': 'Многочленные дробные производные по времени (α, β₁, β₂) и пространству (γ).',
        'color': 'green',
    },
]
    ...
    works = [
        {
            ...
            "name": "АЛГЕБРА ВА АНАЛИЗНИНГ ДОЛЗАРБ МАСАЛАЛАРИ МАВЗУСИДАГИ РЕСПУБЛИКА ИЛМИЙ-АМАЛИЙ АНЖУМАНИ",
        ...
        },
        {
            ...
            "type": "(tezis 3) Buxoro-24.05.2023",
        },
        {
        ...
        },
        {
            ...
            "name": "ЧИСЛЕННОЕ МОДЕЛИРОВАНИЕ ПРОЦЕССА АНОМАЛЬНОГО ПЕРЕНОСА ВЕЩЕСТВА ВО ФРАКТАЛЬНОЙ СРЕДЕ",
            "type": "(maqola) 2023 Mexanika muammolari",
        },
        {
            "url": "(maqola) 2023_Samdu axborotnoma.pdf",
            "name": "АНАЛИЗ ПЕРЕНОСА ВЕЩЕСТВА В ПОРИСТОЙ СРЕДЕ НА ОСНОВЕ ДИФФУЗИОННОГО УРАВНЕНИЯ С МНОГО-ЧЛЕННЫМИ ДРОБНЫМИ ПРОИЗВОДНЫМИ",
            "type": "(maqola) 2023 Samdu axborotnoma",
        },
        {
        ...
        },
        {
            ...
            "type": "(scopus) Computation Adsorption",
        },
    ]
...
def get_item_description(request, item_id):
    ...
    return render(request, 'item_description.html', {'error': True})
...
def result(request):
    if request.method == 'POST':
        ...
        if options == 1:
            ...
            if form.is_valid():
                ...
                for k in range(tmax):
                    for i in range(n + 1):
                        s1 = 0
                        for l1 in range(k):
                            s1 += (Cim[l1 + 1, i] - Cim[l1, i]) * (
                                (k - l1 + 1)**(1 - alpha) - (k - l1)**(1 - alpha)
                            )
                        Cim[k + 1, i] = (
                            Cim[k, i] - s1
                            + (gamma(2 - alpha) * tau**alpha / Rim) * (w * (Cm[k, i] - Cim[k, i]) - A2 * Cim[k, i])
                        )
                    for i in range(1, n):
                        if (bet + 1) == 2:
                            ...
                            s01 = 0
                            for l in range(i):
                                s01 += ((l + 1)**(2 - (bet + 1)) - l**(2 - (bet + 1))) * (
                                    Cm[k, i + 1 - l] - 2 * Cm[k, i - l] + Cm[k, i - 1 - l]
                                )
                        Cm[k + 1, i] = (
                            tau * tetm * Dm * F1 * s01
                            - tau * vm * tetm * (Cm[k, i + 1] - Cm[k, i - 1]) / (Rm * 2 * h)
                            - (tau * w / Rm) * (Cm[k, i] - Cim[k, i])
                            - (tau * A1 / Rm) * Cm[k, i]
                            + Cm[k, i]
                        )
                    Cm[k + 1, n] = Cm[k + 1, n - 1]
                ...
                static_dir = _get_data_dir()
                _save_csv(os.path.join(static_dir, 'Cm.csv'),  list(Cm[tmax, :]))
                _save_csv(os.path.join(static_dir, 'Cim.csv'), list(Cim[tmax, :]))
                _save_csv(os.path.join(static_dir, 'C2.csv'),  list(Cm2))
                _save_csv(os.path.join(static_dir, 'C3.csv'),  list(Cm3))
                _save_csv(os.path.join(static_dir, 'C4.csv'),  list(Cm4))

                return JsonResponse({
                    'x': x, 'y': y,
                    'Cm': Cm.tolist(), 'Cim': Cim.tolist(),
                    'Cm2': Cm2.tolist(), 'Cm3': Cm3.tolist(), 'Cm4': Cm4.tolist(),
                    'Cim2': Cim2.tolist(), 'Cim3': Cim3.tolist(), 'Cim4': Cim4.tolist(),
                    'tmax': tmax, 'tau': tau,
                })
            ...
            C0     = float(request.POST.get('C0',    0.1))
            Dm     = float(request.POST.get('Dm',    1e-4))
            n      = int(request.POST.get('N',       20))
            tau    = float(request.POST.get('tau',   0.5))
            h      = float(request.POST.get('h',     0.01))
            tmax   = int(request.POST.get('tmax',    900))
            ...
            for k in range(tmax):
                for i in range(1, n):
                    s1 = s2 = s3 = 0
                    for l1 in range(k):
                        diff = C[l1 + 1, i] - C[l1, i]
                        s1 += diff * ((k - l1 + 1)**(1 - alpha)  - (k - l1)**(1 - alpha))
                        s2 += diff * ((k - l1 + 1)**(1 - betta1) - (k - l1)**(1 - betta1))
                        s3 += diff * ((k - l1 + 1)**(1 - betta2) - (k - l1)**(1 - betta2))
                    if gam == 2:
                        s01 = C[k, i + 1] - 2 * C[k, i] + C[k, i - 1]
                    else:
                        s01 = 0
                        for l in range(i):
                            s01 += (C[k, i - 1 - l] - 2 * C[k, i - l] + C[k, i + 1 - l]) * (
                                (l + 1)**(2 - gam) - l**(2 - gam)
                            )
            ...
            static_dir = _get_data_dir()
            _save_csv(os.path.join(static_dir, 'C1.csv'), list(C2))
            _save_csv(os.path.join(static_dir, 'C2.csv'), list(C3))
            _save_csv(os.path.join(static_dir, 'C3.csv'), list(C4))

            return JsonResponse({
                'C': C.tolist(), 'C2': C2.tolist(), 'C3': C3.tolist(), 'C4': C4.tolist(),
                'x': x, 'y': y, 'tmax': tmax,
            })
...
def export_csv(request, filename):
    raw = request.GET.get('Cm', '')
    Cm = np.array(raw.split(','), dtype=float)
    output = BytesIO()
    df = pd.DataFrame(Cm)
...
def extract_math_from_pdf(file_path):
    ...
    return re.findall(r'\$\$?(.*?)\$\$?', pdf_text, re.DOTALL)
...
def calculate_data(C0, vm, Dm, n, tau, h, tmax, w, tetim, tetm, gam, alpha, bet):
    ...
    for k in range(tmax):
        for i in range(n + 1):
            ...
            Cim[k + 1, i] = (
                Cim[k, i] - s1 + (gamma(2 - alpha) * tau**alpha * w) * Cm[k, i] / (gam * tetim)
            ) / A
        ...
        for i in range(1, n):
            if bet == 2:
                ...
                s01 = 0
                for l in range(i):
                    s01 += ((l + 1)**(2 - bet) - l**(2 - bet)) * (
                        Cm[k, i + 1 - l] - 2 * Cm[k, i - l] + Cm[k, i - 1 - l]
                    )
            Cm[k + 1, i] = (
                (tau * Dm * s01) / (gamma(3 - bet) * h**bet)
                - tau * vm * (Cm[k, i] - Cm[k, i - 1]) / h
                - gam * tau * tetim * s1 / (tetm * gamma(2 - alpha) * tau**alpha)
                * (Cim[k + 1, i] - Cim[k, i])
                + Cm[k, i]
            )
        Cm[k + 1, n] = Cm[k + 1, n - 1]
